      ...
      try {
        ...
        setDeferrals((prev) => {
          ...
          const hasCreatorApproved =
            updated.creatorApprovalStatus === "approved";
          const hasCheckerApproved =
            updated.checkerApprovalStatus === "approved";
  ...
  const handleConfirmApproval = async () => {
    ...
    try {
      ...
      if (hasCheckerApproved) {
        message.warning(
          "This deferral has already been approved by the checker",
        );
      ...
      console.log(
        "Calling approveByChecker API for deferral:",
        selectedDeferral._id,
      );
      ...
      if (updatedDeferral && updatedDeferral._id) {
        ...
        console.log(
          "Setting deferrals state, new length:",
          updatedDeferrals.length,
        );
        ...
        throw new Error(
          "Invalid response from server - no deferral data returned",
        );
      }
    } catch (error) {
      console.error("Error approving deferral:", error);
      message.error(
        error?.message || error?.toString() || "Failed to approve deferral",
      );
    } finally {
      setActionLoading(false);
    }
            ...
            if (
              ...
              selectedDeferral.history.forEach((h) => {
                ...
                const isApprovalComment =
                  h.action === "approved" ||
                  comment.toLowerCase().includes("approved by");
                const isRejectionComment =
                  h.action === "rejected" ||
                  comment.toLowerCase().includes("rejected by");
                const isReturnComment =
                  h.action === "returned" ||
                  comment.toLowerCase().includes("returned for rework");

                let displayComment = comment;
                if (isApprovalComment) {
                  const match =
                    comment.match(/^Approved by [^:]+:\s*(.*)/i) ||
                    comment.match(/^Approved by [^:]+(.*)/i);
                  if (match && match[1]) {
